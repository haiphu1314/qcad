 ...
 * @ Modified time: 2024-08-02 12:24:20
 * @ Description:
 */
                    ...
                    for (int kc = 0; kc < inputq_size; kc++)
                    {
                        ...
                        { // Unroll outer loop by 2
                            ...
                            for (kx = 0; kx < kernel_size - 1; kx += 2)
                            {
                                ...
                                if (padded_x_next < 0 || padded_y < 0 || padded_x_next >= input_width || padded_y >= input_height)
                                {
                                    cnt_zero += 1;
                                }
                                else
                                {
                                    int result_bit_next = input_quant[kc][padded_y][padded_x_next].b ^ layer->weights_b[co][kc][ky][kx + 1];
                                    cnt_minus_one += bitCount(result_bit_next);
                                }

                                // Handle the remaining element if ky is unrolled by 2
                                if (ky + 1 < kernel_size)
                                {
                                    int padded_y_next = padded_y + dilation;
                                    if (padded_x < 0 || padded_y_next < 0 || padded_x >= input_width || padded_y_next >= input_height)
                                    {
                                        cnt_zero += 1;
                                    }
                                    else
                                    {
                                        int result_bit = input_quant[kc][padded_y_next][padded_x].b ^ layer->weights_b[co][kc][ky + 1][kx];
                                        cnt_minus_one += bitCount(result_bit);
                                    }

                                    if (padded_x_next < 0 || padded_y_next < 0 || padded_x_next >= input_width || padded_y_next >= input_height)
                                    {
                                        cnt_zero += 1;
                                    }
                                    else
                                    {
                                        int result_bit_next = input_quant[kc][padded_y_next][padded_x_next].b ^ layer->weights_b[co][kc][ky + 1][kx + 1];
                                        cnt_minus_one += bitCount(result_bit_next);
                                    }
                                }
                            }

                            // Handle the remaining element if kernel_size is odd
                            if (kx < kernel_size)
                            {
                                int padded_x = (x * stride + kx * dilation - padding) * dilation;
                                int padded_y = (y * stride + ky * dilation - padding) * dilation;
                    ...
                    for (int kc = 0; kc < inputq_size; kc++)
                    {
                        for (int ky = 0; ky < kernel_size; ky++)
                        {
                            ...
                            for (kx = 0; kx < kernel_size - 1; kx += 2)
                            {
                                ...
                                {
                                    int weight_next = layer->weights_b[co][kc][ky][kx + 1];
                                    int i_weight_next = ~weight_next;
                                    int result_bit0_next = (input_quant[kc][padded_y][padded_x_next].t.bit_1 & i_weight_next) | (input_quant[kc][padded_y][padded_x_next].t.bit_0 & weight_next);
                                    int result_bit1_next = (input_quant[kc][padded_y][padded_x_next].t.bit_1 & weight_next) | (input_quant[kc][padded_y][padded_x_next].t.bit_0 & i_weight_next);
                                    cnt_minus_one += bitCount(result_bit0_next);
                                    cnt_one += bitCount(result_bit1_next);
                                }
                            }
                    ...
                    for (int kc = 0; kc < inputq_size; kc++)
                    {
                        for (int ky = 0; ky < kernel_size; ky++)
                        {
                            ...
                            for (kx = 0; kx < kernel_size - 1; kx += 2)
                            {
                                ...
                                {
                                    int result_bit0_next = (input_quant[kc][padded_y][padded_x_next].t.bit_1 & layer->weights_t0[co][kc][ky][kx + 1]) | (input_quant[kc][padded_y][padded_x_next].t.bit_0 & layer->weights_t1[co][kc][ky][kx + 1]);
                                    int result_bit1_next = (input_quant[kc][padded_y][padded_x_next].t.bit_1 & layer->weights_t1[co][kc][ky][kx + 1]) | (input_quant[kc][padded_y][padded_x_next].t.bit_0 & layer->weights_t0[co][kc][ky][kx + 1]);
                                    cnt_minus_one += bitCount(result_bit0_next);
                                    cnt_one += bitCount(result_bit1_next);
                                }
                            }
...
// #pragma omp parallel for collapse(3)
    ...
    int weights_size = output_channel * input_channel * kernel_size * kernel_size;

    float* input_flat = (float*)malloc(input_size * sizeof(float));
    float* weights_flat = (float*)malloc(weights_size * sizeof(float));
    float* output_flat = (float*)malloc(output_channel * output_height * output_width * sizeof(float));

    // Chuyển input sang mảng phẳng
    #pragma omp parallel for collapse(3)
    for (int kc = 0; kc < input_channel; kc++)
    {
        for (int ih = 0; ih < input_height; ih++)
        {
            ...
            {
                input_flat[kc * input_height * input_width + ih * input_width + iw] = input[kc][ih][iw];
            }
        }
    }

    // Chuyển weights sang mảng phẳng
    ...
    for (int co = 0; co < output_channel; co++)
    {
        for (int kc = 0; kc < input_channel; kc++)
        {
            for (int ky = 0; ky < kernel_size; ky++)
            {
                ...
                {
                    weights_flat[co * input_channel * kernel_size * kernel_size + kc * kernel_size * kernel_size + ky * kernel_size + kx] = layer->weights_f[co][kc][ky][kx];
                }
            }
        }
    }

    // Tính toán tích chập bằng cách sử dụng BLAS (SGEMM)
    #pragma omp parallel for collapse(3)
    for (int co = 0; co < output_channel; co++)
    {
        for (int y = 0; y < output_height; y++)
        {
            for (int x = 0; x < output_width; x++)
            {
                int ih_start = y * stride - padding;
                int iw_start = x * stride - padding;
                float sum = 0.0;
                ...
                for (int kc = 0; kc < input_channel; kc++)
                {
                    for (int ky = 0; ky < kernel_size; ky++)
                    {
                        for (int kx = 0; kx < kernel_size; kx++)
                        {
                            int ih = ih_start + ky * dilation;
                            int iw = iw_start + kx * dilation;

                            if (ih >= 0 && ih < input_height && iw >= 0 && iw < input_width)
                            {
                                sum += input_flat[kc * input_height * input_width + ih * input_width + iw] * weights_flat[co * input_channel * kernel_size * kernel_size + kc * kernel_size * kernel_size + ky * kernel_size + kx];
                            }
                        }
                    }
                }
                output_flat[co * output_height * output_width + y * output_width + x] = sum;
            }
        }
    }

    // Chuyển output phẳng sang mảng output 3D
    #pragma omp parallel for collapse(3)
    for (int co = 0; co < output_channel; co++)
    {
        for (int y = 0; y < output_height; y++)
        {
            for (int x = 0; x < output_width; x++)
            {
                output[co][y][x] = output_flat[co * output_height * output_width + y * output_width + x];
            }
        }
    }
    ...
    free(output_flat);
        ...
        //                         int iw_next = iw + layer->dilation; // Next index for unrolling

        //                         if (ih >= 0 && ih < input_height && iw >= 0 && iw < input_width)
        //                         {
        //                             sum += input[kc][ih][iw] * layer->weights_f[co][kc][ky][kx];
        //                         }
        //                         if (ih >= 0 && ih < input_height && iw_next >= 0 && iw_next < input_width)
        //                         {
        //                             sum += input[kc][ih][iw_next] * layer->weights_f[co][kc][ky][kx + 1];
        //                         }
        //                     }
